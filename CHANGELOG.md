...
- Time-stepping accuracy characterization: `tests/test_time_accuracy.py` verifies
  by self-convergence that the solver is first-order in time (the error in the
  final profile scales like the step length), and a **Numerical accuracy** page
  on the documentation site (`docs/accuracy.md`, figure regenerated by
  `docs/accuracy_figure.py`) shows how per-step and final-answer error grow with
  the time step and uses it as the benchmark motivating a second-order-in-time
  scheme ([#16](https://github.com/MNiMORPH/GRLP/issues/16)).
