...
- Time-stepping accuracy characterization: `tests/test_time_accuracy.py` verifies
  ...
  (`docs/accuracy.md`, figure regenerated by `docs/accuracy_figure.py`) shows how
  per-step and final-answer error grow with the time step and compares the two
  schemes ([#16](https://github.com/MNiMORPH/GRLP/issues/16)).
...
- **`LongProfile` split into `Segment` + a 1-D wrapper** (breaking). A single
  ...
  that **does not solve itself**. `LongProfile` is now a lightweight 1-D
  convenience wrapper that *composes* one `Segment` and the one-edge `Network`
  that solves it, forwarding configuration/data to the segment and defining the
  operations that need the network walk plus the 1-D analyses
  (`evolve_threshold_width_river`, `compute_Q_s`, `slope_area`, diffusivity, the
  gain/lag spectral suite, and the closed-form `analytical_threshold_width*`).
  (`Segment.set_niter` is dropped: Picard iteration count is a solve parameter,
  set on the `Network`; `LongProfile.set_niter` sets it on the owned network.) **Usage rule: 1-D → `grlp.LongProfile()`;
