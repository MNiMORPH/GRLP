...
ax.invert_xaxis()
plt.tight_layout()
plt.savefig('WW-sand-Network-Graph.svg')
plt.show()
...
def rivplot(_color, _linewidth=4, _mainalpha=1.):
    for lp in net.list_of_LongProfile_objects:
        ...
        plt.plot(-lp.x, lp.z, '-', color=_color, linewidth=_linewidth, alpha=_mainalpha)#, label=lp.)

#_transalpha = '.5'
#_finalpha = '1.'

###################################################
# 100 YEARS OF CHANGE (WILL UPDATE FOR MORE YEARS #
###################################################
...
_color = '0.7'
rivplot(_color, _linewidth=4)

_color = '0.'
# 10 years of evolution under constant bankfull conditions
for ts in range(10):
    net.evolve_threshold_width_river_network(nt=1, dt=dt*1000)
    rivplot(_color, _linewidth=1, _mainalpha=0.5)

_color = '0.'
rivplot(_color, _linewidth=2, _mainalpha=0.5)
for ts in range(10):
    net.evolve_threshold_width_river_network(nt=1, dt=dt*1000)
    rivplot(_color, _linewidth=1, _mainalpha=0.5)
rivplot(_color, _linewidth=2, _mainalpha=0.5)
...
_color = 'b'
# Raise base level
base_level_rise_rate = 0.001 # m / yr
dt_yr = 1000.
for ts in range(4):
    net.update_z_ext_external_downstream(z_bl + base_level_rise_rate * dt_yr * (ts + 1))    
    net.evolve_threshold_width_river_network(nt=1, dt=dt*dt_yr)
    rivplot(_color, _linewidth=1, _mainalpha=0.5)
rivplot(_color, _linewidth=2, _mainalpha=0.5)
...
#plt.savefig('Ev10k_BLR4k.svg')
plt.savefig('Ev10k_BLR4k_double_size.svg')
plt.show()
