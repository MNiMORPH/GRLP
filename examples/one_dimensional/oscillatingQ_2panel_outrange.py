...
for Qperiod_kyr in [20, 40, 100, 400]:
  ...
  for i in range(nt):
      ...
      # Boundary-augmented profile: the model is node-based and exposes no
      # padded array, so reconstruct the two ghost elevations explicitly --
      # upstream from the fixed boundary slope S0 (recomputed each step, never
      # stale), outlet at base level z_bl.
