...
class LongProfile(object):
    ...
    def set_bcr_Dirichlet(self):
        ...
        self.bcr = self.z_bl * ( self.C1[-1] / self.dx_ext_2cell[0][-1] * 7/3. \
                       * (1/self.dx_ext[0][-2] + 1/self.dx_ext[0][-1])/2. \
                       + self.dQ_2cell[0][-1]/self.Q[-1] )
                      # !!!!!!!!!!!!!!!!!
                      # dQ_2cell --> dQ_2cell[0]. Expecting list! HACK.
    ...
    def set_bcl_Neumann_RHS(self):
        ...
        #                    - self.dQ_2cell[0]/self.Q[0]/self.dx_ext_2cell[0] )
        # !!!C0!!!
        ...
        self.bcl = self.dx_ext_2cell[0][0] * self.S0 * \
                            ...
                            * ( 7/3./self.dx_ext[0][0]
                            - self.dQ_2cell[0][0]/self.Q[0]/self.dx_ext_2cell[0][0] )
                            # !!!!!!!!!!!!!!!!!
                            # dQ_2cell --> dQ_2cell[0]. Expecting list! HACK.
    ...
    def build_matrices(self):
        ...
        self.left = -self.C1 / self.dx_ext_2cell[0] \
                        * ( (7/3.)/self.dx_ext[0][:-1]
                        - self.dQ_2cell[0]/self.Q/self.dx_ext_2cell[0] )
...
class Network(object):
    ...
    def update_dQ_2cell(self):
        ...
        require these to be separated into a dQ_2cell for each river-segment
        ...
        for lp in self.list_of_LongProfile_objects:
            lp.dQ_2cell = []
            for Q_ext_array in lp.Q_ext:
                lp.dQ_2cell.append( Q_ext_array[2:] - Q_ext_array[:-2] )
    ...
    def initialize(self,
        ...
        for lp in segments:
            ...
            lp.z = z[i]
            # !!!!!!!!!!!!!!!!!!!
            # Need to manage dQ_2cell in network
            # TO DO HERE
            # lp.set_Q( Q = Q[i] )
