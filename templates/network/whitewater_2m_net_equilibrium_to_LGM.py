...
with open("/home/awickert/Dropbox/Papers/InProgress/WhitewaterEvolution/GIS/whitewater-3DEP-sand-smoothed-coarsened.json") as f:
    data = json.load(f)
...
def rivplot(_color, _linewidth=4):
    for lp in net.list_of_LongProfile_objects:
        ...
        if len( lp.downstream_segment_IDs ) > 0:
            ...
            plt.plot(-lp.x_ext[0][-2:], lp.z_ext[0][-2:], '-', color=_color, linewidth=_linewidth, alpha=.5)
        plt.plot(-lp.x, lp.z, '-', color=_color, linewidth=_linewidth, alpha=1.)#, label=lp.)

# Equilibrate
net.evolve_threshold_width_river_network(nt=10, dt=dt*1000)
rivplot('0')

# NOW LET'S PERTURB IT
# 32--20,000 years: aggradation. Let's say 10 kyr-ish
# 50 m aggradation? So 5 mm/yr? Then incision afterwards.
# Elevations will be too high, but won't worry about that.

# Rate is limited with GRLP. Sigh. Maybe could increase effective Q and
# therefore increase input sediment. Yeah, sure, let's do it.

# base_level_rise_rate = 0.005 # m / yr

# For ease here, just 20 meters?
base_level_rise_rate = 0.002 # m / yr
...
for ts in range(161):
    ...
    # Aggrade first
    if ts < 101:
        _z_bl = z_bl + base_level_rise_rate * ts_yr * ts
...
# Rapid incision: meltwater from lakes.
# 1000 years and 30 m of incision: 30 mm/yr
base_level_fall_rate = 0.03 # m / yr
_z_bl_initial = _z_bl
# 100-yr time steps
ts_yr = 100.
for ts in range(61):
...
ax.set_xlabel('Distance from river mouth [m]', fontsize=26)
