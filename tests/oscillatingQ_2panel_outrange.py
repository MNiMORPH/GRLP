...
plt.figure()
plt.plot(lp.x, zall_max[0]-zall_min[0], '.7', linewidth=4, label='20 kyr cycle')
plt.plot(lp.x, zall_max[1]-zall_min[1], '.4', linewidth=4, label='40 kyr cycle')
plt.plot(lp.x, zall_max[2]-zall_min[2], '.1', linewidth=4, label='100 kyr cycle')
...
alphas = [1,1,1]
#alphas = [.5,.5,.5]
labels = ['20 kyr cycle', '40 kyr cycle', '100 kyr cycle']

plt.figure()
for i in [2, 1, 0]:
  plt.fill_between(lp.x, zall_max[i] - np.mean((zall_max[i], zall_min[i]), axis=0), zall_min[i] - np.mean((zall_max[i], zall_min[i]), axis=0), color=colors[i], linewidth=1, alpha=alphas[i], label=labels[i])
plt.xlabel('Downstream distance [km]', fontsize=16)
plt.ylabel('Maximum elevation difference\nAcross one cycle [m]', fontsize=16)
#plt.title( str(int(np.round(Qperiod/(1000*3.15E7)))) + ' kyr cycle', fontsize=16, fontweight='bold')
plt.legend()
